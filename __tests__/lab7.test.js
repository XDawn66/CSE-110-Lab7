describe("Basic user flow for Website", () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await button.click();
    let expect_text = "Remove from Cart";
    const buttonTextHandle = await button.getProperty("innerText");
    const button_text = await buttonTextHandle.jsonValue();
    expect(expect_text).toBe(button_text);
    await button.click(); //essential for the next test!
  }, 2500);
  ...
  it("Checking number of items in cart on screen", async () => {
    ...
    const buttonTextHandle = await cartCount.getProperty("innerText");
    const button_text = await buttonTextHandle.jsonValue();
    expect(button_text).toBe("20");
  }, 20000);
  ...
  it("Checking number of items in cart on screen after removing from cart", async () => {
    ...
    const buttonTextHandle2 = await cartCount2.getProperty("innerText");
    const button_text2 = await buttonTextHandle2.jsonValue();
    expect(button_text2).toBe("0");
  }, 20000);
